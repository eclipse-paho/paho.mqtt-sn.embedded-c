...
Topic* Topics::getTopicByName(const MQTTSN_topic* topicid)
{
    Topic* p = _first;
    char* ch = topicid->alt.string.data;

    string sname = string(ch, ch + topicid->alt.string.len);
    while (p)
    {
...
Topic* Topics::getTopicById(const MQTTSN_topic* topicid)
{
    Topic* p = _first;

    while (p)
    {
        if (p->_type == topicid->type && p->_topicId == topicid->alt.alias)
        {
            return p;
        }
        p = p->_next;
...
// For MQTTSN_TOPIC_TYPE_NAME */
Topic* Topics::add(const MQTTSN_topic* topicid)
{
    if (topicid->type != MQTTSN_TOPIC_TYPE_NAME)
    {
        return 0;
    }
    ...
    string name(topicid->alt.string.data, topicid->alt.string.len);
...
Topic* Topics::add(const char* topicName, uint16_t id)
{
    ...
    if (id == 0)
    {
        topic->_type = MQTTSN_TOPIC_TYPE_NAME;
...
Topic* Topics::match(const MQTTSN_topic* topicid)
{
    if (topicid->type != MQTTSN_TOPIC_TYPE_NAME)
    {
        return 0;
    }
    string topicName(topicid->alt.string.data, topicid->alt.string.len);
...
void Topics::eraseNormal(void)
{
    ...
    while (topic)
    {
        if (topic->_type == MQTTSN_TOPIC_TYPE_NAME)
        {
            next = topic->_next;
            if (_first == topic)
            {
...
/*=====================================
 Class TopicIdMap
 =====================================*/
TopicIdMapElement::TopicIdMapElement(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic)
{
    _msgId = msgId;
    _topicId = topicId;
    _type = topic->type;
    ...
    if (_type == MQTTSN_TOPIC_TYPE_NAME)
    {
        if (strchr(topic->alt.string.data, '#') != 0 || strchr(topic->alt.string.data, '+') != 0)
        {
            _wildcard = 1;
        }
    }
...
TopicIdMapElement* TopicIdMap::add(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic)
{
    if (_cnt > _maxInflight * 2 || (topicId == 0 && topic->type != MQTTSN_TOPIC_TYPE_NAME))
    {
        return 0;
    }
    if (getElement(msgId))
