...
void Gateway::initialize(int argc, char** argv)
{
    ...
    if (getParam("ProtectionSenderId", param) == 0)
    {
        int idlen = (int)strlen(param) / 2;
        if (idlen > 8) idlen = 8;
        for (int i = 0; i < idlen; i++)
        {
            unsigned int byte = 0;
            sscanf(param + i * 2, "%02x", &byte);
            _params.protectionSenderId[i] = (uint8_t)byte;
        }
    }

    if (getParam("ProtectionKey", param) == 0)
    {
        int keyLen = (int)strlen(param) / 2;
        if (keyLen > 32) keyLen = 32;
        for (int i = 0; i < keyLen; i++)
        {
            unsigned int byte = 0;
            sscanf(param + i * 2, "%02x", &byte);
            _params.protectionKey[i] = (uint8_t)byte;
        }
        _params.protectionKeyLen = (uint8_t)keyLen;
    }
