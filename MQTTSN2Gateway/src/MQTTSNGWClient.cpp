...
Client::Client()
{
    ...
    memset(_protectionSenderId, 0, sizeof(_protectionSenderId));
    memset(_protectionKey, 0, sizeof(_protectionKey));
    _protectionKeyLen = 0;
    _forwarder = nullptr;
...
void Client::setWaitedPubTopicId(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic)
{
    _waitedPubTopicIdMap.add(msgId, topicId, topic);
}

void Client::setWaitedSubTopicId(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic)
{
...
void Client::setKeepAlive(MQTTSNPacket* packet)
{
    ...
    {
        _keepAliveMsec = param.keepAlive * 1000UL;
...
void Client::setClientId(MQTTSN_string id)
{
    if (_clientId)
    {
        free(_clientId);
    }
    _clientId = (char*) calloc(id.len + 1, 1);
    if (_clientId && id.data)
    {
        memcpy(_clientId, id.data, id.len);
    }
}

void Client::setWillTopic(MQTTSN_string willTopic)
{
    if (_willTopic)
    {
        free(_willTopic);
    }
    _willTopic = (char*) calloc(willTopic.len + 1, 1);
    if (_willTopic && willTopic.data)
    {
        memcpy(_willTopic, willTopic.data, willTopic.len);
    }
}

void Client::setWillMsg(MQTTSN_string willMsg)
{
    if (_willMsg)
    {
        free(_willMsg);
    }
    _willMsg = (char*) calloc(willMsg.len + 1, 1);
    if (_willMsg && willMsg.data)
    {
        memcpy(_willMsg, willMsg.data, willMsg.len);
    }
}
...
void Client::setPingReqPacketId(uint16_t id)
{
    _pingReqPacketId = id;
}

uint16_t Client::getPingReqPacketId(void)
{
    return _pingReqPacketId;
}

void Client::setProtection(const uint8_t* senderid, const uint8_t* key, uint8_t keylen)
{
    memcpy(_protectionSenderId, senderid, 8);
    if (keylen > (uint8_t)sizeof(_protectionKey))
        keylen = (uint8_t)sizeof(_protectionKey);
    memcpy(_protectionKey, key, keylen);
    _protectionKeyLen = keylen;
    _protectionActive = true;
}

bool Client::isProtectionActive(void) const
{
    return _protectionActive;
}

const uint8_t* Client::getProtectionSenderId(void) const
{
    return _protectionSenderId;
}

const uint8_t* Client::getProtectionKey(void) const
{
    return _protectionKey;
}

uint8_t Client::getProtectionKeyLen(void) const
{
    return _protectionKeyLen;
}
