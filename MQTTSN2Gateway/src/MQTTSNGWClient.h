...
public:
    ...
    void setWaitedPubTopicId(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic);
    void setWaitedSubTopicId(uint16_t msgId, uint16_t topicId, MQTTSN_topic* topic);
    ...
    void setClientId(MQTTSN_string id);
    void setWillTopic(MQTTSN_string willTopic);
    void setWillMsg(MQTTSN_string willmsg);
    ...
    void setPingReqPacketId(uint16_t id);
    uint16_t getPingReqPacketId(void);

    /* PROTECTION (MQTT-SN 2.0 Section 3.17) */
    void setProtection(const uint8_t* senderid, const uint8_t* key, uint8_t keylen);
    bool isProtectionActive(void) const;
    const uint8_t* getProtectionSenderId(void) const;
    const uint8_t* getProtectionKey(void) const;
    uint8_t getProtectionKeyLen(void) const;
...
private:
    ...
    uint16_t _pingReqPacketId;
    bool _protectionActive;
    uint8_t _protectionSenderId[8];
    uint8_t _protectionKey[32];
    uint8_t _protectionKeyLen;
