...
bool ClientList::createList(const char* fileName, int type)
{
    ...
    MQTTSN_string clientId = {false, 0, nullptr};
    ...
    if ((fp = fopen(fileName, "r")) != 0)
    {
        while (fgets(buf, MAX_CLIENTID_LENGTH + 254, fp) != 0)
        {
            ...
            clientId.data = strdup(id.c_str());
            clientId.len = (uint16_t)id.length();
            clientId.islen8 = false;
...
Client* ClientList::getClient(MQTTSN_string* clientId)
{
    _mutex.lock();
    Client* client = _firstClient;
    const char* clID = clientId->data;

    while (client != nullptr)
    {
        if (strncmp((const char*) client->getClientId(), clID, clientId->len) == 0)
        {
            _mutex.unlock();
            return client;
        }
...
Client* ClientList::createClient(SensorNetAddress* addr, MQTTSN_string* clientId, int type)
{
    return createClient(addr, clientId, false, false, type);
}

Client* ClientList::createClient(SensorNetAddress* addr, MQTTSN_string* clientId, bool unstableLine, bool secure, int type)
{
    ...
    {
        MQTTSN_string dummyId = {false, 0, nullptr};
        dummyId.data = strdup("");
...
Client* ClientList::createPredefinedTopic(MQTTSN_string* clientId, string topicName, uint16_t topicId, bool aggregate)
{
    if (topicId == 0)
    {
    ...
    if (strcmp(clientId->data, common_topic) == 0)
    {
