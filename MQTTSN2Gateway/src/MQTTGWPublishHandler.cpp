...
void MQTTGWPublishHandler::handlePublish(Client* client, MQTTGWPacket* packet)
{
    ...
    topicId.alt.string.len = pub.topiclen;
    topicId.alt.string.data = pub.topic;
    ...
    if (tp)
    {
        ...
        topicId.alt.string.len = pub.topiclen;
        topicId.alt.string.data = pub.topic;
        topicId.alt.alias = tp->getTopicId();
    }
    else
    {
        ...
        if (id > 0)
        {
            ...
            MQTTSN_string topicName = {false, 0, nullptr};
            topicName.len = topicId.alt.string.len;
            topicName.data = topicId.alt.string.data;

            uint16_t regackMsgId = client->getNextSnMsgId();
            regPacket->setREGISTER(1, id, regackMsgId, &topicName);
...
void MQTTGWPublishHandler::handlePuback(Client* client, MQTTGWPacket* packet)
{
    ...
    if (topicId)
    {
        ...
        mqttsnPacket->setPUBACK((uint16_t) ack.msgId, 0);
...
void MQTTGWPublishHandler::handleAck(Client* client, MQTTGWPacket* packet, int type)
{
    ...
    if (client->isActive() || client->isAwake())
    {
        ...
        if (type == PUBREC)
        {
            mqttsnPacket->setPUBREC((uint16_t) ack.msgId, 0);
        }
        else if (type == PUBREL)
        {
            mqttsnPacket->setPUBREL((uint16_t) ack.msgId, 0);
        }
        else if (type == PUBCOMP)
        {
            mqttsnPacket->setPUBCOMP((uint16_t) ack.msgId, 0);
        }
