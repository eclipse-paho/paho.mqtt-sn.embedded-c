...
void Adapter::setup(const char* adpterName, AdapterType adapterType)
{
    ...
    MQTTSN_string id = {false, 0, nullptr};
    MQTTSN_string idSecure = {false, 0, nullptr};

    string name = string(adpterName);
    id.data = const_cast<char*>(name.c_str());
    id.len = (uint16_t)name.length();
    string nameSecure = string(adpterName) + "-S";
    idSecure.data = const_cast<char*>(nameSecure.c_str());
    idSecure.len = (uint16_t)nameSecure.length();
...
void Proxy::checkConnection(Client* client)
{
    if (client->isDisconnect() || (client->isConnecting() && _responseTimer.isTimeup()))
    {
        ...
        options.clientID.data = client->getClientId();
        options.clientID.len = (uint16_t)strlen(client->getClientId());
        options.keepAlive = PROXY_KEEPALIVE_DURATION;
